...
let regexp right_keyword = "typeof" | "new" | "var" | "do" | "case" | "void" | "for" | "switch"
...
and token = lexer
  ...
  | numeric_literal -> let i = float_of_string (utf8_lexeme lexbuf) in c lexbuf; re := false; cut_right := false; NUMBER(i)
  | identifier -> let id = decode_unicode (utf8_lexeme lexbuf) in c lexbuf; re := false; cut_right := false; IDENTIFIER(id)
  | eof -> if !finished then EOF else (finished := true; EOL)
